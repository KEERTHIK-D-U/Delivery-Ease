...
export const confirmOrder = async (req, reply) => {
    try{
        ...
        const delievryPerson = await DeliveryPartner.findById(userId);
        if(!delievryPerson){
...
export const updateOrderStatus = async (req, reply) => {
    try{
        ...
        if (["canceled", "delivered"].includes(order.status)){
            return reply.status(400).send({message: 'Cannot update status of a canceled or delivered order'});
        }

        if (order.deliveryPartner.toString() !== userId) {
            return reply.status(403).send({message: 'You are not assigned to this order'});
        }
